...
describe 'hellofresh service (normal port option)' do
  describe 'definition' do
    describe command 'curl -s http://127.0.0.1:8500/v1/catalog/service/hellofresh' do
      ...
      its(:stdout) { should contain '"ServiceAddress":"127.0.01"' }
  ...
  describe 'HAProxy server backend should be listed and up' do
    ...
    describe command "echo 'show stat' | socat unix-connect:/var/lib/haproxy/stats.sock  stdio | grep hellofresh,hellofresh" do
      its(:exit_status) { should eq 0 }
    end
  end
